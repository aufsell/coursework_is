...
BEGIN
    BEGIN
        -- если значения не изменились - скип
        IF TG_OP = 'UPDATE' AND
           NEW.ibu_pref IS NOT DISTINCT FROM OLD.ibu_pref AND
           NEW.srm_pref IS NOT DISTINCT FROM OLD.srm_pref AND
           NEW.abv_pref IS NOT DISTINCT FROM OLD.abv_pref AND
           NEW.og_pref  IS NOT DISTINCT FROM OLD.og_pref  AND
           NEW.price    IS NOT DISTINCT FROM OLD.price    AND
           NEW.fermentation_type IS NOT DISTINCT FROM OLD.fermentation_type
        THEN
            RETURN NEW;
        END IF;
